...
void OutlineFilter::Excute(const ComPtr<ID3D12GraphicsCommandList>& commandList, const ComPtr<ID3D12RootSignature>& postRootSignature, const ComPtr<ID3D12Resource>& depthStencil, const ComPtr<ID3D12Resource>& renderTarget)
{
	/*
	1. 깊이스텐실 버퍼를 m_buffer0에 복사한다.
	2. 렌더타겟을 m_buffer1에 복사한다.
	3. m_buffer0을 이용하여 소벨필터를 적용해서 외곽선을 검출하고 m_buffer1에 저장한다.
	4. m_buffer1을 렌더타겟으로 복사한다.
	*/

	commandList->SetComputeRootSignature(postRootSignature.Get());
	commandList->SetPipelineState(m_shader->GetPipelineState().Get());

	// 깊이스텐실 버퍼를 m_buffer0에 복사한다.
	...
	commandList->CopyResource(m_buffer0.Get(), depthStencil.Get());
	...
	// 렌더타겟을 m_buffer1에 복사한다.
	commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_buffer1.Get(), D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_COPY_DEST));
	...
	commandList->CopyResource(m_buffer1.Get(), renderTarget.Get());
	commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_buffer1.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_UNORDERED_ACCESS));
